...
All five methods share the same \textbf{safety checks}: every candidate puzzle
is compared against the 554 real NYT puzzles both as a whole 16-word set
\emph{and} group-by-group. If any generated group exactly matches a real NYT
group, we throw that group out and try again. This means we cannot fail the
``don't copy the NYT'' rule --- the system refuses to emit anything that would
trigger it.
